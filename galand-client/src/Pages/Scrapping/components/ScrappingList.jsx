...
import ImageComponent from "./ImageArticle";
import DefaultImg from "../../../assests/default-logo4.png";
import ChatGptAnswer from "./ChatGptAnswer";
import { apiEndpoint } from "config";
import ReverseSearchModal from "Pages/Videos/ReverseSearchModal";
...
class ScrappingList extends Component {
  ...
  getStopWords() {
    if (!this.props.settings.data) return [];
    let stopWordsSetting = this.props.settings.data.filter(
      (setting) => setting._id === "Stop Words"
    )[0];
  ...
  componentDidMount() {
    setTimeout(() => {
      const images = document.querySelectorAll("#inner-html > img ");
      images.forEach((img) => {
        if (img.complete && img.naturalWidth === 0) {
          img.src = DefaultImg;
          img.srcset = DefaultImg;
        }
      });
    }, 4000);
  }
  ...
  componentDidUpdate(prevProps) {
    if (JSON.stringify(prevProps.feed) !== JSON.stringify(this.props.feed))
      setTimeout(() => {
        const images = document.querySelectorAll("#inner-html > img ");
        images.forEach((img) => {
          if (img.complete && img.naturalWidth === 0) {
  ...
  getPropsForDisplayType(display, displayType) {
    return display
      .filter((d) => d.displayType === displayType)
      .map((d) => d.propName);
  }
  ...
  highlightText = (text, searchTerm) => {
    ...
    const fragmentsAnd = searchTerm
      .split("&")
      .map((el) => el.trim())
      .filter((el) => el.length > 0);
    const fragmentsOr = searchTerm
      .split("|")
      .map((el) => el.trim())
      .filter((el) => el.length > 0);
    let textToReturn = text;

    // Function to highlight each fragment
    for (let index = 0; index < fragmentsAnd.length; index++) {
      const searchFragment = fragmentsAnd[index];
      textToReturn = ("" + textToReturn).replace(
        new RegExp(searchFragment, "gi"),
        (match) => `<span class="highlight">${match}</span>`
      );
    }
    for (let index = 0; index < fragmentsOr.length; index++) {
      const searchFragment = fragmentsOr[index];
      textToReturn = ("" + textToReturn).replace(
        new RegExp(searchFragment, "gi"),
        (match) => `<span class="highlight">${match}</span>`
      );
    }
    return <div dangerouslySetInnerHTML={{ __html: textToReturn }} />;
  };
  ...
  render() {
    ...
    const searchTerm = filter && filter.all_text;

    if (!feed || !feed.data || !feed.data.length) return null;
        ...
        {feed.data.map((item, i) => {
          ...
          return (
              ...
              header={
                <div style={{ marginTop: "-20px" }}>
                  <div className="box-header-top">
                    <span className={"label margin-r-5 margin-b-10"}>
                      {item.source}
                    </span>
                    {this.getPropsForDisplayType(display, "tag").map(
                      (key) =>
                        ...
                        item[key] !== null && (
                          <span
                            key={key}
                            className={"label margin-r-5 margin-b-10"}
                          >
                  ...
                  {this.getPropsForDisplayType(display, "header").map(
                    (key, index) =>
                      item[key] && (
                        <h4
                          key={index}
                          className={"text-center text-lg ellepsis"}
                        >
                          <a target={"_blank"} href={item.url}>
                            ...
                            {key === "url"
                              ...
                              : this.highlightText(item[key], searchTerm)}
                          </a>
                        </h4>
                      )
                  )}
                  {this.getPropsForDisplayType(display, "header body").map(
                    (key) => (
                      <h4 key={key}>
                        <b>
                          {this.getLabelToDisplay(
                            key,
                            display,
                            "header body"
                          ) || key}
                    ...
                    {this.getPropsForDisplayType(display, "right tag").map(
                      (key) =>
                        item[key] && (
                          <span
                            key={key}
                            className={"text-muted text-md pull-right"}
                          >
            ...
            >
              ...
              {this.getPropsForDisplayType(display, "body no label").map(
                (key) => {
                  return <p>{this.highlightText(item[key], searchTerm)}</p>;
                }
              )}

              {this.getPropsForDisplayType(display, "image").map((key) =>
                item[key] ? (
                  item[key].includes("lookaside") ? (
                    <a
                      href={item[key]}
                      target={"_blank"}
                      style={{
                    ...
                    >
                      ...
                      >
                        <path
                          fillRule="evenodd"
                          clipRule="evenodd"
                          d="M12 2C12.5523 2 13 2.44772 13 3V17C13 17.5523 12.5523 18 12 18C11.4477 18 11 17.5523 11 17V3C11 2.44772 11.4477 2 12 2Z"
                          fill="currentColor"
                        ></path>
                        <path
                          fillRule="evenodd"
                          clipRule="evenodd"
                          d="M3 16C3.55228 16 4 16.4477 4 17C4 18.6567 5.34328 20 7 20H17C18.6567 20 20 18.6567 20 17C20 16.4477 20.4477 16 21 16C21.5523 16 22 16.4477 22 17C22 19.7613 19.7613 22 17 22H7C4.23872 22 2 19.7613 2 17C2 16.4477 2.44772 16 3 16Z"
                          fill="currentColor"
                        ></path>
                        <path
                          fillRule="evenodd"
                          clipRule="evenodd"
                          d="M6.29196 11.2928C6.68252 10.9023 7.31568 10.9024 7.70617 11.293L11.9991 15.5867L16.2929 11.2929C16.6834 10.9024 17.3166 10.9024 17.7071 11.2929C18.0976 11.6834 18.0976 12.3166 17.7071 12.7071L12.7061 17.7081C12.5186 17.8957 12.2642 18.001 11.9989 18.001C11.7337 18.001 11.4793 17.8956 11.2918 17.708L6.29182 12.707C5.90133 12.3165 5.90139 11.6833 6.29196 11.2928Z"
                          fill="currentColor"
                        ></path>
                      </svg>
                    </a>
                    ...
                    <a href="#">
                      <ReverseSearchModal url={encodeURI(item[key])} />
                      <ImageComponent
                        src={item[key]}
              ...
              )}
              {
                <div className="d-flex justify-content-between margin-t-15 br-top">
                  <div>
                    {this.getPropsForDisplayType(display, "count love").map(
                      (key) => {
                        return (
                          <img
                            width={"25px"}
                            className="margin-r-10 pointer"
                            title={item[key]}
                    ...
                    {this.getPropsForDisplayType(display, "count like").map(
                      (key) => {
                        return (
                          <img
                            width={"25px"}
                    ...
                    {this.getPropsForDisplayType(display, "count sad").map(
                      (key) => {
                        return (
                          <img
                            width={"25px"}
                    ...
                    {this.getPropsForDisplayType(display, "count care").map(
                      (key) => {
                        return (
                          <img
                            width={"25px"}
                    ...
                    {this.getPropsForDisplayType(display, "count angry").map(
                      (key) => {
                        return (
                          <img
                            width={"25px"}
                    ...
                    {this.getPropsForDisplayType(display, "count haha").map(
                      (key) => {
                        return (
                          <img
                            width={"25px"}
                    ...
                    {this.getPropsForDisplayType(display, "count wow").map(
                      (key) => {
                        return (
                          <img
                            width={"25px"}
                  ...
                  <div>
                    {this.getPropsForDisplayType(display, "count share").map(
                      (key) => {
                        return (
                          <>
                            {item[key]}{" "}
              ...
              <div
                className={
                  this.getPropsForDisplayType(display, "body html no label")
                    .length
                    ? "row d-flex flex-wrap"
                    : ""
                }
              >
                {this.getPropsForDisplayType(display, "body html no label").map(
                  (key) => {
                    const match = srcRegex.exec(item[key]);
                    const srcValue = match ? match[1] : null;

                    return (
                      <>
                        <span>
                          <ReverseSearchModal url={srcValue} />
                        </span>
                        <div
                          key={key}
                          hidden={!item[key]}
                          ...
                          dangerouslySetInnerHTML={{
                            __html: String(item[key]).replace(
                              "archive",
                              "store/archive"
                            ),
                          }}
              ...
              {this.getPropsForDisplayType(display, "body html").length ? (
                <dl className="dl-horizontal">
                  {this.getPropsForDisplayType(display, "body html").map(
                    (key) => (
                      <Row
                        hidden={!item[key]}
                        key={key}
              ...
              {this.getPropsForDisplayType(display, "Array_of_reaction").map(
                (key) => {
                  return (
                    <div key={key}>
                      {" "}
              ...
              <div className="d-flex">
                {this.getPropsForDisplayType(display, "count comments").map(
                  (key) => {
                    return (
                      <div
                        style={{
                ...
                {this.getPropsForDisplayType(display, "Array").map((key) => {
                  return (
                    <div key={key}>
                      {<Comments comments={item[key]} item={item} t={t} />}
                    </div>
                  );
                })}
              </div>
