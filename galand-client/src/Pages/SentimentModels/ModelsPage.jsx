...
import { Models, Sentiment } from "modules/ressources";
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    insert: (data, toggleModal) => {
      dispatch(
        Models.insert({...data, source:"sentiment"}, err => {
    ...
    update: (data, toggleModal) =>
      dispatch(
        Models.update({...data, source:"sentiment"}, err => {
          //if no errors we hide the modal
          if (!err) toggleModal();
        })
      ),
    remove: data => dispatch(Models.remove({...data, source:"sentiment"}))
  };
}
